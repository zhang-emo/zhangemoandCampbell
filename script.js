(function() {
    ...
    function updateCallUI() { if (!inCall) return; callTimer.textContent = cmiTime.textContent = formatTime(Math.floor((Date.now()-callStartTime)/1000)); }
    ...
    function endCall() {
        ...
        if (inCall) msgs.push({ id:nid++, senderId:SYS, text:`📞 通话结束，时长 ${formatTime(Math.floor((Date.now()-callStartTime)/1000))}`, timestamp:Date.now() });
        else if (incomingWaiting) msgs.push({ id:nid++, senderId:SYS, text:'📞 未接来电', timestamp:Date.now() });
        else if (isDialing) msgs.push({ id:nid++, senderId:SYS, text:'📞 对方未接听', timestamp:Date.now() });
        saveAllData(); render();
        callWindow.style.display = 'none'; inCall = false; callMinimized = false; incomingWaiting = false; isDialing = false;
        scheduleNextIncomingCall(); render();
    }
    function incomingCall() {
        if (inCall || incomingWaiting || isDialing) { scheduleNextIncomingCall(); return; }
    ...
    function startDialing() {
        ...
        callWindow.style.display = 'flex'; callWindow.style.width = '220px'; callWindow.style.height = 'auto'; callWindow.style.borderRadius = '24px';
        callHeader.style.display = 'flex'; callBody.style.display = 'flex'; cmiBar.style.display = 'none';
        callTitle.textContent = '正在呼叫...'; callTimer.style.display = 'none'; callHangup.style.display = 'none'; incActions.style.display = 'none';
        callAvatar.innerHTML = ctAv && ctAv.startsWith('data:image') ? `<img src="${ctAv}" style="width:100%;height:100%;object-fit:cover">` : ctAv;
        callWindow.style.left = '50%'; callWindow.style.top = '50%'; callWindow.style.transform = 'translate(-50%,-50%)';
        dialTimer = setTimeout(() => { if (Math.random() < 0.6) { isDialing = false; startCall(true); } else endCall(); }, Math.floor(Math.random()*2000)+1000);
    }
    incAccept.onclick = () => { incomingWaiting = false; startCall(true); };
    incAccept.addEventListener('touchend', (e) => { e.preventDefault(); incAccept.onclick(); });
    incHangup.onclick = () => { endCall(); };
    incHangup.addEventListener('touchend', (e) => { e.preventDefault(); endCall(); });
    ...
    callHangup.addEventListener('touchend', (e) => { e.preventDefault(); endCall(); });
    callMin.onclick = (e) => {
        e.stopPropagation();
        if (incomingWaiting || isDialing) return;
        if (!callMinimized) { callWindow.style.width='160px'; callWindow.style.height='40px'; callWindow.style.borderRadius='20px'; callHeader.style.display='none'; callBody.style.display='none'; cmiBar.style.display='flex'; callMinimized=true; }
        else { callWindow.style.width='220px'; callWindow.style.height='auto'; callWindow.style.borderRadius='24px'; callHeader.style.display='flex'; callBody.style.display='flex'; cmiBar.style.display='none'; callMinimized=false; }
    };
    ...
    let dragInfo = null;
    function startDrag(e) { if (incomingWaiting || isDialing) return; const x = e.touches?e.touches[0].clientX:e.clientX, y = e.touches?e.touches[0].clientY:e.clientY; dragInfo = {x:x-callWindow.offsetLeft, y:y-callWindow.offsetTop}; if(e.touches)e.preventDefault(); }
    function onDrag(e) { if(!dragInfo) return; const x = e.touches?e.touches[0].clientX:e.clientX, y = e.touches?e.touches[0].clientY:e.clientY; callWindow.style.left = (x-dragInfo.x)+'px'; callWindow.style.top = (y-dragInfo.y)+'px'; callWindow.style.transform = 'none'; if(e.touches)e.preventDefault(); }
    function endDrag(){ dragInfo=null; }
    callHeader.addEventListener('mousedown', startDrag); callHeader.addEventListener('touchstart', startDrag, {passive:false});
    cmiBar.addEventListener('mousedown', startDrag); cmiBar.addEventListener('touchstart', startDrag, {passive:false});
    document.addEventListener('mousemove', onDrag); document.addEventListener('touchmove', onDrag, {passive:false});
    document.addEventListener('mouseup', endDrag); document.addEventListener('touchend', endDrag);

    function scheduleNextIncomingCall() {
        clearTimeout(incomingCallTimer);
        incomingCallTimer = setTimeout(incomingCall, Math.floor(Math.random()*(20-10+1)*60000) + 10*60000);
    }
    ...
    importHistoryBtn.addEventListener('touchend', (e) => { e.preventDefault(); historyJSONInput.click(); });
            ...
            try {
                ...
                if (Array.isArray(data)) {
                    if (confirm('导入将替换当前聊天记录，确定继续吗？')) {
                        msgs = data;
                        nid = msgs.length ? Math.max(...msgs.map(m => m.id), 0) + 1 : 1000;
                        render(); renderHist(); saveAllData();
                    }
                } else alert('无效的聊天记录文件');
            } catch(ex) { alert('无效的JSON文件'); }
        };
    ...
    function renderGroupBar() {
        ...
        let h = groups.map(g => `<div class="group-tag ${currentGroupFilter===g.id?'active':''}" data-group-id="${g.id}" style="border-color:${g.color}"><span class="group-color" style="background:${g.color}"></span>${g.name}<button class="group-edit-btn" data-edit-group="${g.id}">✎</button>${g.id!=='default'?`<button class="group-edit-btn" data-del-group="${g.id}">✕</button>`:''}</div>`).join('');
    ...
    function exportGroupJSON(groupId) {
        ...
        if (!group) return;
        let data = { text: textCards.filter(c => c.groupId === groupId), emoji: emojiCards, image: imageCards, status: statusCards, groups, exportGroupId: groupId, exportGroupName: group.name };
        let blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); let a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'wordbank_' + group.name + '.json'; a.click();
    }
